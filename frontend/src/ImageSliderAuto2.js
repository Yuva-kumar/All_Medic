...
const ImageSliderAuto2 = (props) => {
    ...
    const NextClick = () => {
        ...
        if (count <= props.ImageData2.length-1) {
            setCount(count+1);
        }
        if (count === props.ImageData2.length-1) {
            setCount(0);
    ...
    useEffect(() => {
        ...
        setSliderProperty((previous) => ({ ...previous, ImageNo: props.ImageData2[count].ImageNo,
             ImageName: props.ImageData2[count].ImageName, ImageSrc: props.ImageData2[count].ImageSrc }));
    }, [count]);
    useEffect(() => {
...
export default ImageSliderAuto2;